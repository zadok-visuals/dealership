...
export default function CarDetailPage() {
  ...
  useEffect(() => {
    async function fetchCar() {
      try {
        ...
        if (data) {
          setCar(data);
        } else if (id && SAMPLE_CARS[id as string]) {
          setCar(SAMPLE_CARS[id as string]);
        }
      } catch (err) {
        console.error("Error fetching car:", err);
        if (id && SAMPLE_CARS[id as string]) {
          setCar(SAMPLE_CARS[id as string]);
        }
      } finally {
        setLoading(false);
  ...
  const displayPrice = currency === 'USD' ? car.price : car.price * 1500;
